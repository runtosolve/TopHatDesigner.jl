...
function define_new_deck_bracing_properties(top_hat_purlin_line)
    ...
    if top_hat_purlin_line.inputs.new_deck_details[1] == "screw-fastened"
        ...
        for i = 1:num_purlin_segments
            ...
            section_index = top_hat_purlin_line.inputs.segments[i][3]

            #Define the material property index associated with purlin segment i.
            material_index = top_hat_purlin_line.inputs.segments[i][4]
...
function analysis(top_hat_purlin_line)
    ...
    top_hat_purlin_line.distortional_demand_to_capacity, eMnd_xx_all, top_hat_purlin_line.Β_distortional_gradient_factor = PurlinLine.calculate_distortional_buckling_demand_to_capacity(top_hat_purlin_line)
